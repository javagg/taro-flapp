...
// import { LayoutService } from "./paragraph";

import { Paragraph, TextStyle } from "@/mtex/canvaskit";
...
/**
 * A concrete implementation of ParagraphBuilder that builds CanvasParagraph.
 */
export class CanvasParagraphBuilder implements ParagraphBuilder {
    ...
    pop(): void {
        if (this._styleStack.length <= 1) {
            throw new Error('Cannot pop the last style from the stack.');
        }
    ...
    build(): Paragraph {
        if (this._styleStack.length !== 1) {
            throw new Error('Style stack is not empty. Did you forget to call pop()?');
        }
